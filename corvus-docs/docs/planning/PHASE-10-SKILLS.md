...
**Status:** Active — Phase 10a–d implementation
...
3. **Browse** allowlisted public registries (skills.sh-compatible API) from the Operator Console, then install only through the same gated path.
...
- Registry browse results are **untrusted advertising**; dry-run shows real `SKILL.md` + computed archive sha256 before commit.
...
| Audit | `skill_browse` / `skill_install` / `skill_deny`; tool phases for `skill_read` / `skill_run`. |
...
| **10d** | Allowlisted registry browse + prepare-install (Console Browse page) |

## API (10d)

- `GET /v1/skills/browse?q=&page=&page_size=` — proxy search
- `GET /v1/skills/browse/{owner}/{repo}/{skill_id}` — detail (+ optional registry content)
- `POST /v1/skills/browse/prepare-install` — `{ owner, repo, skill_id, ref?, dry_run?, allow_scripts? }` → GitHub archive hash + existing install path

Console: `/ui/tools/skills/browse`.

## Non-goals

- Auto-install without allowlist; guest mid-turn download; Engine 3 direct skill execution; scraping HTML marketplaces; Phase 11 mounts / Phase 12 tools.
